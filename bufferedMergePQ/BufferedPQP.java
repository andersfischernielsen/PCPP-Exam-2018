...
public class BufferedPQP implements PQ {
    ...
    BufferedPQP(Parameters pp) {
        ...
        // this is the recursion; look at SerialPQPair.java and Parameters.java (and
        // PQPair.java) for details
        // Paramaters knows if to split recursively, pqPair creates the two recursive
        // instances
    ...
    // Gets the mininmal element. If all the elements of the buffer are taken (that
    // is
    ...
    static public String stringArray(int[] arr, int f, int l) {
        ...
        if (arr != null) {
            for (int i = f; i < l; i++)
                res = res + (" " + (arr[i] == Integer.MAX_VALUE ? ". " : "" + arr[i]));
        }
        return res;
    }

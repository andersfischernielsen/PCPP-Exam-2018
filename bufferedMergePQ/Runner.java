...
public class Runner {
    // for optional command line arguments at variable positions. Very useful for my
    // debugging cycle on the command line; allows java style int constants with _
    public static int useArg(String[] args, int pos, int def) {
        if (args.length > pos)
            return Integer.parseInt(args[pos].replace("_", ""));
        return def;
    }

    // my command line looks like >>> rm -f *.class &&javac Runner.java && java
    // Runner 3445 5_000 4 99
    public static void main(String[] args) {
        ...
        System.out.printf("n %d  s %d, extract %d  bufLen %d  maxDepth %d  cutOff %d%n", n, seed, extract, bufLen,
                maxDepth, cutOff);
        SystemInfo();
        ...
        timePQ(a, extract,
                (x) -> new OneBufferPQ(new Parameters(x, 0, x.length, 0, bufLen, cutOff, maxDepth, false, null)),
                "One Serial");
        timePQ(a, extract,
                (x) -> new OneBufferPQ(new Parameters(x, 0, x.length, 0, bufLen, cutOff, maxDepth, true, null)),
                "One Parallel");
    ...
    // debugging. Adjust to what you need -- or kick it out if it gets in the way
